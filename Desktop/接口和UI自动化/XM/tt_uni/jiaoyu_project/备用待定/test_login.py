...
class TestLogin(unittest.TestCase):
    excel = HandleExcel(os.path.join(data_dir,'teacher1.0.xlsx'), 'login')
    #读取用例的数据
    ...
    @list_data(cases)
    def test_verify(self, item):            #item用来接收用例数据
        ...
        reponse = requests.request(method, url, data=datas, headers=self.headers)
        res = (reponse.json())
